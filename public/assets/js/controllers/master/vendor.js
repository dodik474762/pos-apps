let Vendor = {
    ...
    cancel: (elm, e) => {
        e.preventDefault();
        window.location.href = url.base_url(Vendor.module()) + "/";
    },
    ...
    submit: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            let params = Vendor.getPostInput();
            $.ajax({
                type: "POST",
                dataType: "json",
                data: params,
            ...
            columns: [
                ...
                // },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (deleteAction == 1) {
                            html += `<button type="button" data_id="${row.id}" onclick="Vendor.delete(this, event)" class="btn btn-danger editable-cancel btn-sm waves-effect waves-light"><i class="bx bx-trash-alt"></i></button>`;
                        }
                        return html;
                    },
                },
    ...
    confirmDelete: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(Vendor.moduleApi()) + "confirmDelete",
            headers: {
                "X-CSRF-TOKEN": Vendor.csrf_token(),
            },
            beforeSend: () => {
