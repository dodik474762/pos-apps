...
let PurchaseReturn = {
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (row.status.toLowerCase() == "draft") {
                            html += `<button type="button" data_id="${row.id}" onclick="PurchaseReturn.postJurnal(this, event)" class="btn btn-warning editable-post btn-sm waves-effect waves-light" title="Buat Jurnal Purchase Return"><i class="bx bx-book-alt"></i></button>&nbsp;`;
                        }
                        if (deleteAction == 1) {
                            if (row.status.toLowerCase() == "draft") {
    ...
    postJurnal: (elm, e) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(PurchaseReturn.moduleApi()) + "showModalPostJurnal",
            headers: {
                "X-CSRF-TOKEN": PurchaseReturn.csrf_token(),
            },
            beforeSend: () => {
    ...
    confirmPostJurnal: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(PurchaseReturn.moduleApi()) + "postJurnal",
            headers: {
                "X-CSRF-TOKEN": PurchaseReturn.csrf_token(),
            },
            beforeSend: () => {
