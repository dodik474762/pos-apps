...
let CreditNote = {
    ...
    cancel: (elm, e) => {
        e.preventDefault();
        window.location.href = url.base_url(CreditNote.module()) + "/";
    },

    add: (elm, e) => {
        e.preventDefault();
        window.location.href = url.base_url(CreditNote.module()) + "add";
    },
    ...
    getPostInput: () => {
        let data = {
            id: $("#id").val() || null,
            credit_note_number: $("#credit_note_number").val() || null,
            credit_note_date: $("#credit_note_date").val() || null,
            customer_id: $("#customer_id").attr("data_id") || null,
            invoice_id: $("#invoice_id").attr("data_id") || null,
            note_type: $("#note_type").val() || null,
            total_amount: parseFloat($("input#total_amount").val()) || 0,
            discount_amount: parseFloat($("input#discount_amount").val()) || 0,
            tax_amount: parseFloat($("input#tax_amount").val()) || 0,
            reason: $("#credit_reason").val() || null,
            items: CreditNote.getPostItem(),
        };
    ...
    submit: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            let params = CreditNote.getPostInput();
            $.ajax({
                type: "POST",
                dataType: "json",
                data: params,
                url: url.base_url(CreditNote.moduleApi()) + "submit",
                headers: {
                    "X-CSRF-TOKEN": CreditNote.csrf_token(),
                },
                beforeSend: () => {
    ...
    getData: async () => {
        ...
        var data = tableData.DataTable({
            ...
            ajax: {
                url: url.base_url(CreditNote.moduleApi()) + `getData`,
                type: "POST",
                headers: {
                    "X-CSRF-TOKEN": CreditNote.csrf_token(),
                },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (deleteAction == 1) {
                            if (row.status == "DRAFT") {
                                html += `<button type="button" data_id="${row.id}" onclick="CreditNote.delete(this, event)" class="btn btn-danger editable-cancel btn-sm waves-effect waves-light"><i class="bx bx-trash-alt"></i></button>`;
                            }
                        }
                        return html;
                    },
    ...
    delete: (elm, e) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "delete",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
            beforeSend: () => {
    ...
    confirmDelete: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "confirmDelete",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
            beforeSend: () => {
    ...
    posted: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "posted",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
            beforeSend: () => {
    ...
    showModalCustomer: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "showModalCustomer",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
    ...
    showModalInvoice: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "showModalInvoice",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
    ...
    getDataCustomer: () => {
        ...
        var data = tableData.DataTable({
            ...
            ajax: {
                url: url.base_url(CreditNote.moduleApiCustomer()) + `getData`,
                type: "POST",
                headers: {
                    "X-CSRF-TOKEN": CreditNote.csrf_token(),
                },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var html = "";
                        html += `<a href='' code="${row.code}" nama_customer="${row.nama_customer}" onclick="CreditNote.pilihDataCust(this, event)" data_id="${row.id}" class="btn btn-info editable-submit btn-sm waves-effect waves-light"><i class="bx bx-edit"></i></a>&nbsp;`;
                        return html;
                    },
                },
            ],
    ...
    getDataInvoice: () => {
        ...
        var data = tableData.DataTable({
            ...
            ajax: {
                url: url.base_url(CreditNote.moduleApi()) + `getDataInvoice`,
                type: "POST",
                data: params,
                headers: {
                    "X-CSRF-TOKEN": CreditNote.csrf_token(),
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        var html = "";
                        html += `<a href='' invoice_number="${row.invoice_number}" onclick="CreditNote.pilihDataInvoice(this, event)" data_id="${row.id}" class="btn btn-info editable-submit btn-sm waves-effect waves-light"><i class="bx bx-edit"></i></a>&nbsp;`;
                        return html;
                    },
                },
            ],
    ...
    pilihDataInvoice: (elm, e) => {
        ...
        CreditNote.getProductInvoice(data_id);
    },
    ...
    getProductInvoice: (invoice) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(CreditNote.moduleApi()) + "getProductInvoice",
            headers: {
                "X-CSRF-TOKEN": CreditNote.csrf_token(),
            },
    ...
    calcDiscRow: (elm) => {
        ...
        const UOM_CONVERSION = CreditNote.getDataUomConversion();
        const DATA_DISKON = CreditNote.getDataDiskon();
        const DATA_DISKON_FREE = CreditNote.getDataDiskonFreeGood();

        // Konversi qty input ke satuan terkecil
        const qtySmallest = CreditNote.convertToSmallest(
            UOM_CONVERSION,
            productId,
            satuanId,
            qty
        ...
        const applicable = DATA_DISKON.find((d) => {
            ...
            const minSmall = CreditNote.convertToSmallest(
                UOM_CONVERSION,
                d.product_id,
                d.unit_id,
                d.min_qty
            );
            const maxSmall = CreditNote.convertToSmallest(
                UOM_CONVERSION,
                d.product_id,
                d.unit_id,
                d.max_qty
                    ...
                    <tr class="input freegood" data-free-for="${productId}">
                        <td>
                            <div class="input-group">
                                <button class="btn btn-outline-secondary" type="button" disabled onclick="CreditNote.showDataProduct(this)">Free</button>
                        ...
                        <td class="text-center"><button type="button" class="btn btn-sm btn-danger" disabled onclick="CreditNote.removeRow(this)"><i class="bx bx-gift"></i></button></td>
                    </tr>
                `;
    ...
    editReload: () => {
        ...
        if (id != "") {
            ...
            resultProduct.forEach((item) => {
                ...
                CreditNote.showDiscountProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
                CreditNote.showDiscountFreeProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
                CreditNote.showQtySmallestProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
...
$(function () {
    CreditNote.setSelect2();
    CreditNote.getData();
    CreditNote.editReload();
});
