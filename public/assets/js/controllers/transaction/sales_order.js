...
let SalesOrder = {
  ...
  getDataDiskon: () => {
    const table_diskon = $("table#table-data-diskon").find("tbody").find("tr");
    const DATA_DISKON = [];
    if (table_diskon.length > 0) {
      $.each(table_diskon, (index, elm) => {
        ...
        const discount_value = isNaN(parseFloat($(elm).attr("discount_value")))
          ? 0
      ...
      for (let index = 0; index < promoHeaders.length; index++) {
        ...
        if (mixOk && today >= promoHeader.date_start) {
          ...
          if (discount_kategori == "nominal") {
            ...
            console.log('diskon kategori nominal',rawSubtotal, minNominal, maxNominal);
            ...
            if (kelipatan == "1") {
              ...
              if (rawSubtotal >= minNominal && pengaliKelipatanFreegood > 0) {
        ...
        if (discount_kategori === "nominal") {
          ...
          pengali = kelipatanNominal == 0 ? 1 : Math.floor(rawSubtotal / kelipatanNominal);
          pengaliFix = pengali == 1 ? 1 : Math.floor(pengali / promoHeader.min_qty);
        } else {
          // pengali berdasarkan qty terkecil (existing)
          pengali = kelipatanSmall == 0 ? 1 : Math.floor(qtySmallestAllProduct / kelipatanSmall);
          pengaliFix = pengali == 1 ? 1 : Math.floor(pengali / promoHeader.min_qty);
        }
        ...
        if (promoApplicable && promoFree.length) {
          promoFree.forEach((free) => {
            ...
            const exists =
              tr.next('tr.freegood[data-free-for="' + productId + '"]').length >
              0;
  ...
  getPromoFreeProducts: (className) => {
    const free = [];
    $("#table-data-promo-product-free tbody tr." + className).each(function () {
      free.push({
  ...
  editReload: () => {
    ...
    if (id != "") {
      ...
      resultProduct.forEach((item) => {
        ...
        SalesOrder.showDiscountProduct([product_id], [product_name], [unit_id]);
        ...
        SalesOrder.showPromoItem([product_id], [product_name], [unit_id], 0);
      });
