...
let TerimaUang = {
    ...
    cancel: (elm, e) => {
        e.preventDefault();
        window.location.href = url.base_url(TerimaUang.module()) + "/";
    },

    add: (elm, e) => {
        e.preventDefault();
        window.location.href = url.base_url(TerimaUang.module()) + "add";
    },

    getPostItem: () => {
        const table = $("table#table-data tbody tr.input");
        let result = [];
    ...
    getPostInput: () => {
        let data = {
            visit_date: $("#filterDate").val() || null,
            salesman: $("#salesman").val() || null,
            items: TerimaUang.getPostItem(),
        };

        return data;
    },

    submit: (elm, e, type = 'pending') => {
        e.preventDefault();
        ...
        if (validation.runWithElement(form)) {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: params,
                url: url.base_url(TerimaUang.moduleApi()) + "submit",
                headers: {
                    "X-CSRF-TOKEN": TerimaUang.csrf_token(),
                },
                beforeSend: () => {
    ...
    posted: (elm, e) => {
        ...
        if (validation.runWithElement(form)) {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: params,
                url: url.base_url(TerimaUang.moduleApi()) + "posted",
                headers: {
                    "X-CSRF-TOKEN": TerimaUang.csrf_token(),
                },
                beforeSend: () => {
    ...
    getData: async () => {
        ...
        var data = tableData.DataTable({
            ...
            ajax: {
                url: url.base_url(TerimaUang.moduleApi()) + `getData`,
                type: "POST",
                headers: {
                    "X-CSRF-TOKEN": TerimaUang.csrf_token(),
                },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (deleteAction == 1) {
                            if (row.status == "DRAFT") {
                                html += `<button type="button" data_id="${row.id}" onclick="TerimaUang.delete(this, event)" class="btn btn-danger editable-cancel btn-sm waves-effect waves-light"><i class="bx bx-trash-alt"></i></button>`;
                            }
                        }
                        return html;
                    },
    ...
    delete: (elm, e) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(TerimaUang.moduleApi()) + "delete",
            headers: {
                "X-CSRF-TOKEN": TerimaUang.csrf_token(),
            },
            beforeSend: () => {
    ...
    confirmDelete: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(TerimaUang.moduleApi()) + "confirmDelete",
            headers: {
                "X-CSRF-TOKEN": TerimaUang.csrf_token(),
            },
            beforeSend: () => {
    ...
    showModalDO: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(TerimaUang.moduleApi()) + "showModalDO",
            headers: {
                "X-CSRF-TOKEN": TerimaUang.csrf_token(),
            },
    ...
    getDataDo: () => {
        ...
        var data = tableData.DataTable({
            ...
            ajax: {
                url: url.base_url(TerimaUang.moduleApi()) + `getDataDo`,
                type: "POST",
                headers: {
                    "X-CSRF-TOKEN": TerimaUang.csrf_token(),
                },
            ...
            columns: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        onclick="TerimaUang.pilihDataDo(this, event)"
    ...
    pilihDataDo: (elm, e) => {
        ...
        TerimaUang.getDoDetail(data_id);
    },
    ...
    getDoDetail: (do_id) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(TerimaUang.moduleApi()) + "getDoDetail",
            headers: {
                "X-CSRF-TOKEN": TerimaUang.csrf_token(),
            },
    ...
    calcDiscRow: (elm) => {
        ...
        const UOM_CONVERSION = TerimaUang.getDataUomConversion();
        const DATA_DISKON = TerimaUang.getDataDiskon();
        const DATA_DISKON_FREE = TerimaUang.getDataDiskonFreeGood();

        // Konversi qty input ke satuan terkecil
        const qtySmallest = TerimaUang.convertToSmallest(
            UOM_CONVERSION,
            productId,
            satuanId,
            qty
        ...
        const applicable = DATA_DISKON.find((d) => {
            ...
            const minSmall = TerimaUang.convertToSmallest(
                UOM_CONVERSION,
                d.product_id,
                d.unit_id,
                d.min_qty
            );
            const maxSmall = TerimaUang.convertToSmallest(
                UOM_CONVERSION,
                d.product_id,
                d.unit_id,
                d.max_qty
            ...
            if (!exists) {
                const freeRow = `
                    ...
                    "Free Product"
                        ...
                        <td class="text-center"><button type="button" class="btn btn-sm btn-danger" disabled onclick="TerimaUang.removeRow(this)"><i class="bx bx-gift"></i></button></td>
                    </tr>
                `;
    ...
    editReload: () => {
        ...
        if (id != "") {
            ...
            resultProduct.forEach((item) => {
                ...
                TerimaUang.showDiscountProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
                TerimaUang.showDiscountFreeProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
                TerimaUang.showQtySmallestProduct(
                    [product_id],
                    [product_name],
                    [unit_id]
                );
...
$(function () {
    TerimaUang.setSelect2();
    TerimaUang.editReload();
});
