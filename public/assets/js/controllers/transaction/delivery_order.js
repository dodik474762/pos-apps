...
let DeliveryOrder = {
                    ...
                    render: function (data, type, row) {
                        ...
                        if (updateAction == 1) {
                            if (row.status == "CONFIRMED" || row.status == "PACKED") {
                                html += `<button type="button" data_id="${row.id}" onclick="DeliveryOrder.postJurnal(this, event)" class="btn btn-warning editable-post btn-sm waves-effect waves-light" title="Buat Jurnal HPP"><i class="bx bx-book-alt"></i></button>&nbsp;`;
                            }
                        }
                        return html;
                    },
    ...
    postJurnal: (elm, e) => {
        ...
        $.ajax({
            ...
            dataType: "html",
            data: params,
            url: url.base_url(DeliveryOrder.moduleApi()) + "showModalPostJurnal",
            headers: {
                "X-CSRF-TOKEN": DeliveryOrder.csrf_token(),
            },
            beforeSend: () => {
    ...
    confirmPostJurnal: (elm) => {
        ...
        $.ajax({
            ...
            dataType: "json",
            data: params,
            url: url.base_url(DeliveryOrder.moduleApi()) + "postJurnal",
            headers: {
                "X-CSRF-TOKEN": DeliveryOrder.csrf_token(),
            },
            beforeSend: () => {
