...
function generateCodePengajuanLimitTop()
{
    $no = 'PLT' . strtoupper(date('m')) . date('y');
    $data = DB::table('customer_limit_top')->where('code', 'LIKE', '%' . $no . '%')->orderBy('code', 'desc')->get()->toArray();
...
function canInvoiceProcessCL($customer, $total = 0, $exclude_id = 0)
{
    ...
    $totalOutstanding = $exclude_id == 0 || $exclude_id == '' ? 0 : $totalOutstanding->sum(DB::raw('total_amount - amount_paid'));
    ...
    if ($totalWithNew > $credit_limit) {
        return [
            'status' => false,
            'message' => 'Customer telah mencapai batas kredit maksimal (Rp ' . number_format($credit_limit, 0, ',', '.') . '), total outstanding ' . $totalOutstanding . ' + invoice baru (Rp ' . number_format($totalWithNew, 0, ',', '.') . ') ID : ' . $exclude_id,
        ];
    }
