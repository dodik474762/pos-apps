    ...
    public function getData(Request $request)
    {
        ...
        $datadb = SalesOrderHeader::from('sales_order_headers as m')
            ->select([
                ...
                'sih.amount_paid',
                'sih.invoice_number',
                'spd.allocated_amount as payment_amount',
                'spd.outstanding_amount',
                'sph.payment_date',
                'sih.id as invoice_id',
                'sph.payment_code',
                DB::raw('(spd.outstanding_amount - spd.allocated_amount) as sisa_payment')
            ])
            ...
            ->leftJoin('sales_payment_detail as spd', function ($q) {
                return $q->on('spd.invoice_id', 'sih.id')->whereNull('spd.deleted');
            })
            ->leftJoin('sales_payment_header as sph', function ($q) {
                return $q->on('sph.id', 'spd.payment_id')->whereNull('sph.deleted');
            })
            ...
            // ->where('sih.invoice_number', 'SI08260494')
            ->whereNull('m.deleted')
            ->whereNull('sih.deleted');
        if (isset($data['customer_id'])) {
            if ($data['customer_id'] != '') {
                $datadb->where('sih.customer_id', $data['customer_id']);
            }
        }
            ...
            if (isset($_POST['order'][0]['column'])) {
                switch ($_POST['order'][0]['column']) {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        $datadb->orderBy('sih.invoice_number', $_POST['order'][0]['dir'])
                            ->orderBy('sph.payment_date');
                        break;
                    case 4:
