...
class TemplateMenuController extends Controller
{
    ...
    public function getAccurateMenuTree()
    {
        $user_id = Session::get('user_id');
        ...
        return $this->buildMenuTree($dataMenu, 0);
    }

    protected function buildMenuTree($rows, $parent = 0)
    {
        $tree = [];
        ...
        foreach ($rows as $row) {
            if ($row->parent == $parent) {
                $node = (array) $row;
                $node['children'] = $this->buildMenuTree($rows, $row->id);
                $tree[] = $node;
            }
        }
