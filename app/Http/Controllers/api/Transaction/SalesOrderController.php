...
use App\Models\Transaction\SalesOrderPromoItem;
use Carbon\Carbon;
...
class SalesOrderController extends Controller
{
    ...
    public function submit(Request $request)
    {
        ...
        try {
            ...
            foreach ($items as $item) {
                ...
                $freeGoods = [];
                // echo '<pre>';
                // print_r($calculatePromo['result_items']);
                // die;
                $totalDiscItemAmount = 0;
                $totalDiscItemPercent = 0;
                ...
                foreach ($calculatePromo['result_items'] as $promo) {
                    ...
                    $totalDiscItemAmount += $promoItemFound['discountAmount'] ?? 0;
                    $totalDiscItemPercent += $promoItemFound['discountPercent'] ?? 0;
                ...
                $item['disc_amount'] = $totalDiscItemAmount;
    ...
    public function sync(Request $request)
    {
        ...
        try {
            ...
            $header->total_amount    = 0;
            $header->platform        = 'mobile';
            $header->photo_path      = $dbpathlampOutlet == '' ? null : $dbpathlampOutlet . $fileOutletName;
            ...
            $header->tax_id       = $taxId;   // fix: sebelumnya tertukar
            $header->save();
            $soId = $header->id;
   ...
   public function getAverageTransaction(Request $request)
    ...
    public function saveInvoice($params)
    {
        ...
        try {
            ...
            $data['total_amount'] = ROUND($data['total_amount'], 0);
            $data['discount_amount'] = ROUND($data['discount_amount'], 0);
            ...
            foreach ($data['items'] as $item) {
                ...
                $detail->product_id = $item['product_id'];
                $detail->qty = $item['qty'];
                $detail->price = ROUND($item['unit_price'], 0);
                $detail->discount = ROUND($item['discount_amount'], 0);
            ...
            $discountHeaderSo = $so->discount_amount == '' ? 0 : ROUND($so->discount_amount, 0);
            $header->discount_amount = ROUND($discountHeaderSo, 0);
            ...
            /*check credit limit */
            $cl_check = canInvoiceProcessCL($cust_id, $header->total_amount - $header->discount_amount, 0);
            if (!$cl_check['status']) {
                DB::rollBack();
                return response()->json([
                    'is_valid' => false,
