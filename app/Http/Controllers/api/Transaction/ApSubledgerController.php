...
class ApSubledgerController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        $datadb = DB::table('ap_invoices as ai')
            ->select([
                'ai.*',
                'v.code as supplier_code',
                'v.nama_vendor',
            ])
            ->join('vendor as v', 'v.id', 'ai.supplier_id')
        ...
        if ($request->filled('supplier_id')) {
            $datadb->where('ai.supplier_id', $request->supplier_id);
        }
        ...
        $data['recordsTotal'] = DB::table('ap_invoices')->whereNull('deleted_at')->count();

        if ($request->filled('search.value')) {
            $keyword = $request->input('search.value');
            $datadb->where(function ($query) use ($keyword) {
                $query->where('ai.invoice_no', 'LIKE', '%' . $keyword . '%');
                $query->orWhere('v.code', 'LIKE', '%' . $keyword . '%');
        ...
        if ($request->filled('order.0.column')) {
            $columns = ['ai.id', 'ai.invoice_no', 'v.nama_vendor', 'ai.invoice_date', 'ai.due_date', 'ai.invoice_amount', 'ai.outstanding_amount', 'ai.status'];
        ...
        foreach ($data['data'] as &$row) {
            $row->detail_url = url('transaksi/ap_subledger/supplier-ledger') . '?supplier_id=' . $row->supplier_id;
        }
    ...
    public function getLedgerData(Request $request)
    {
        ...
        foreach ($invoices as $inv) {
            $ledger->push([
                ...
                'debit' => 0,
                'credit' => (float) $inv->invoice_amount,
                'balance' => 0,
                'status' => $inv->status,
