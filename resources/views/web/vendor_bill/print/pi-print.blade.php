...
<head>
    <meta charset="utf-8">
    <title>Purchase Invoice - {{ $data->invoice_number }}</title>
    <style>
        body {
...
<body>
    ...
    <table class="header-table" style="width:100%;">
        <tr>
            ...
            <td style="text-align:right;">
                <h4 style="margin:0; padding:0;">PURCHASE INVOICE</h4>
    ...
    <table class="no-border" style="width:100%;">
        <tr>
            <td><strong>Kode PI:</strong> {{ $data->invoice_number }}</td>
            <td style="padding-left:40px;"><strong>Tanggal Invoice:</strong> {{ date('d/m/Y', strtotime($data->invoice_date)) }}</td>
        </tr>
        <tr>
        ...
        <tbody>
            @foreach ($data->items as $i => $item)
                <tr>
                    ...
                    <td class="text-right">{{ $item->po_detail->tax_rate }}%/{{ number_format($item->po_detail->tax_amount, 0, ',', '.') }}</td>
