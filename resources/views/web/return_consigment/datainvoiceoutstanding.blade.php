@foreach ($data as $item)
     <tr data_id="" invoice_detail_id="{{ $item->id }}">
        ...
        <td><input type="number" qty_invoice="{{ $item->qty }}" onkeyup="CreditNote.changeQtyRetur(this)" id="qty_return" class="form-control" min="1" max="{{ $item->outstanding_can_return }}" value="{{ $item->outstanding_can_return }}"></td>
        ...
        <td class="text-center">
            <button type="button" class="btn btn-sm btn-danger" onclick="CreditNote.removeRow(this)">
